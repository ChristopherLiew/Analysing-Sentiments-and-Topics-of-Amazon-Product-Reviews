...
amz = pd.read_csv(
    "./Amazon product reviews dataset/Amazon_product_review_with_sent.csv"
)
...
amz_Y = amz["sentiment"].map(
    {"Positive": 1, "Neutral": 0, "Negative": -1}
)  # Encode sentiments
amz_X_train, amz_X_test, amz_Y_train, amz_Y_test = train_test_split(
    amz_X, amz_Y, test_size=0.25, random_state=42, shuffle=True
)

# Train Set
amz_train_set = pd.merge(
    left=amz_X_train, right=amz_Y_train, left_index=True, right_index=True
)
# Test Set
amz_test_set = pd.merge(
    left=amz_X_test, right=amz_Y_test, left_index=True, right_index=True
)
# Save for future use
...
amz_neu = amz_train_set.loc[
    amz_train_set["sentiment"] == 0, ["reviews.text", "sentiment"]
]
amz_neg = amz_train_set.loc[
    amz_train_set["sentiment"] == -1, ["reviews.text", "sentiment"]
]
# Majority class
amz_pos = amz_train_set.loc[
    amz_train_set["sentiment"] == 1, ["reviews.text", "sentiment"]
]
...
def data_augment_synonym(data, num_augments):
    ...
    for row in working_data.itertuples():
        ...
        for i in range(num_augments):
            ...
            working_data = working_data.append(
                pd.Series({"reviews.text": aug_text, "sentiment": review_sent}),
                ignore_index=True,
            )
...
amz_syn_aug_data.to_csv(
    "../Amazon product reviews dataset/Synonym_augmented_data/amazon_synaug_train.csv",
    index=False,
)
