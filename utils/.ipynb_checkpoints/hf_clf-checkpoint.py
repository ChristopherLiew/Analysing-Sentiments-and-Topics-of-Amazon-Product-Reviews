...
def compute_clf_metrics(eval_pred: EvalPrediction) -> Callable[[EvalPrediction], Dict]:
    ...
    metrics["eval/accuracy"] = acc_metric.compute(
        predictions=predictions, references=labels
    )

    metrics["eval/f1"] = f1_metric.compute(
        predictions=predictions, references=labels, average="macro"
    )

    metrics["eval/precision"] = prec_metric.compute(
        predictions=predictions, references=labels, average="macro"
    )

    metrics["eval/recall"] = rec_metric.compute(
        predictions=predictions, references=labels, average="macro"
    )
