...
def build_lsi_model(
    ...
    lsi_model = models.lsimodel.LsiModel(
        corpus=corpus, id2word=dictionary, num_topics=num_topics, chunksize=100
    )
    if save:
        try:
            input_path = input(
    ...
    if compute_coherence:
        try:
            ...
            coherence_model_lsi = models.CoherenceModel(
                model=lsi_model,
                corpus=corpus,
...
lsi_pos, lsi_pos_umass_score = build_lsi_model(
...
params_grid = {"num_topics": list(range(2, 15, 1))}
