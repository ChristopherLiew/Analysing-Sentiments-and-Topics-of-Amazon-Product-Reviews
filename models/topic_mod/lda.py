...
pd.DataFrame(pd.Series(clean_amz), columns=["reviews"]).to_csv(
    "data/topic_data/proc_text.csv"
)
...
pd.DataFrame(pd.Series(clean_amz_ref), columns=["reviews_pos_filtered"]).to_csv(
    "data/topic_data/pos_filtered_proc.csv"
)
...
amz_ngrams = pd.concat(
    [
        pd.DataFrame({"reviews_bigrams": amz_bigram}),
        pd.DataFrame({"reviews_trigrams": amz_trigram}),
        amz_full.sentiment,
    ],
    axis=1,
)
...
id2wordPos.filter_extremes(
    no_below=20, no_above=0.5
...
def tune_lda_model(
    corpus, dictionary, hyperparameters, val_set_size=0.75, coherence_metric="u_mass"
):
    # Try randomised search
    results = {"num_topics": [], "alpha": [], "beta": [], "results": []}
...
lda_pos, lda_pos_umass_score = build_lda_model(
    corpus=corpus_tfidf_pos, dictionary=id2wordPos, num_topics=10, save=False
)
...
pos_results = tune_lda_model(
    corpus=corpus_tfidf_pos,
    val_set_size=1.0,
...
lda_opt_pos, score = build_lda_model(
    corpus_tfidf_pos, id2wordPos, num_topics=3, alpha=0.01, beta=0.9099999999999999
)
pprint(lda_opt_pos.print_topics())

# 2) Neutral
# Get baseline coherence score
lda_neu, lda_neu_umass_score = build_lda_model(
...
lda_opt_neu, score_neu = build_lda_model(
    corpus_tfidf_neu, id2wordNeu, num_topics=3, alpha=0.01, beta=0.01
)
pprint(lda_opt_neu.print_topics())

# 3) Negative
# Get baseline coherence score
lda_neg, lda_neg_umass_score = build_lda_model(
...
lda_opt_neg, score_neg = build_lda_model(
    corpus_tfidf_neg, id2wordNeg, num_topics=9, alpha=0.91, beta=0.91
)
