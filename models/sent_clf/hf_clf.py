...
# 3) Abstract and create util functions for handling Training and Inference Pipeline
...
def predict(model_name: Optional[str],  # Check if we can infer from model artifact
            ...
            wandb_entity: str = 'chrisliew',
            wandb_proj_name: str = 'amz-sent-analysis',
            num_labels: int = 3,  # Check if we can infer this from the configs
    ...
    with wandb.init(project=wandb_proj_name) as run:
        ...
        if inf_data is not None:
            ...
            ds_wnb_fp = os.path.join(wandb_entity,
                                     wandb_proj_name,
                                     wandb_proj_name,
                                     '_datasets:latest')
