...
raw_train_text, raw_train_labels = (
    raw_train_ds["text_processed"].to_list(),
...
raw_test_text, raw_test_labels = (
    raw_test_ds["text_processed"].to_list(),
...
VAL_SPLIT = 0.2
val_set_size = int(
    (len(amz_train_ds) + len(amz_test_ds)) * VAL_SPLIT
)  # 20% of entire DS
...
def build_bert_classifier(preprocessing_model, bert_model):
    ...
    bert_preprocessing_layer = hub.KerasLayer(
        preprocessing_model, name="bert_preprocessing"
    )  # Truncate input to 128

    encoder_inputs = bert_preprocessing_layer(text_input)

    bert_encoder = hub.KerasLayer(
        bert_model, trainable=True, name="BERT_encoder"
    )  # Fine tune BERT params
...
classifier_model.compile(loss=loss, optimizer=optimizer, metrics=metric)
...
model_checkpoint_cb = ModelCheckpoint(
    "models/saved_models/%s" % MODEL_NAME.split(".")[0], save_best_only=True
)
...
classification_rep = pd.DataFrame(
    classification_report(y_pred=y_pred, y_true=raw_test_labels, output_dict=True)
)
