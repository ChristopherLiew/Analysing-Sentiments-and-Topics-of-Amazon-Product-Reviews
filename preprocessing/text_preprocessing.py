...
def lemmatize_and_remove_stopwords(text, nlp, tokenize=True, is_lower_case=True):
    tokens = nlp(text)
    tokens = [token.text.strip() for token in tokens]
    if is_lower_case:
        filtered_tokens = [token for token in tokens if token not in all_stopwords]
        filtered_tokens = [
            token for token in filtered_tokens if token not in all_punctuation
        ]
    else:
        filtered_tokens = [
            token.lower() for token in tokens if token not in all_stopwords
        ]
        filtered_tokens = [
            token for token in filtered_tokens if token not in all_punctuation
        ]
...
def preprocess_text(
    ...
    return pd.DataFrame(
        {"reviews.text": processed_corpus, "reviews.rating": labels}
    ).dropna()
